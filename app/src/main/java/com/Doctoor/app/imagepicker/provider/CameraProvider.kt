package com.Doctoor.app.imagepicker.provider
...
import com.Doctoor.app.imagepicker.ImagePickerActivity
import com.Doctoor.app.imagepicker.util.FileUtil
import com.Doctoor.app.imagepicker.util.IntentUtils
import com.Doctoor.app.imagepicker.util.PermissionUtil
import com.Doctoor.app.imagepicker.util.PermissionUtil.isPermissionGranted
import java.io.File
...
class CameraProvider(activity: ImagePickerActivity) : BaseProvider(activity) {
    ...
    override fun onFailure() {
        ...
        if (mCameraFile?.exists()!!) {
            mCameraFile?.canonicalFile?.delete();
            if (mCameraFile?.exists()!!) {
                applicationContext.deleteFile(mCameraFile?.name)
            }
        }
    }
}