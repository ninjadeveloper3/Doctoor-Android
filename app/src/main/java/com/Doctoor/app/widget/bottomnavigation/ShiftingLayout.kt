...
class ShiftingLayout(context: Context) : ItemsLayoutContainer(context) {
    ...
    init {
        totalChildrenSize = 0
        maxActiveItemWidth =
            resources.getDimensionPixelSize(R.dimen.bbn_shifting_maxActiveItemWidth)
        minActiveItemWidth =
            resources.getDimensionPixelSize(R.dimen.bbn_shifting_minActiveItemWidth)
    ...
    private fun populateInternal(menu: MenuParser.Menu) {
        ...
        if (totalWidth > screenWidth) {
            ...
            ratio = (Math.round(ratio * ROUND_DECIMALS)
                .toDouble() / ROUND_DECIMALS).toFloat() + RATIO_MIN_INCREASE

            itemWidthMin =
                Math.max(maxInactiveItemWidth * ratio, minInactiveItemWidth.toFloat()).toInt()
            itemWidthMax = (maxActiveItemWidth * ratio).toInt()

            if (itemWidthMin * (menu.itemsCount - 1) + itemWidthMax > screenWidth) {
                itemWidthMax =
                    screenWidth - itemWidthMin * (menu.itemsCount - 1) // minActiveItemWidth?
