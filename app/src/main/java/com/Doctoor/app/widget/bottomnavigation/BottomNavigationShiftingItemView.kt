...
internal class BottomNavigationShiftingItemView(
    ...
    private val paddingTopItem: Int =
        resources.getDimensionPixelSize(R.dimen.bbn_shifting_item_padding_top)
    private val paddingBottomActive: Int =
        resources.getDimensionPixelSize(R.dimen.bbn_shifting_item_padding_bottom_active)
    private val paddingBottomInactive: Int =
    ...
    init {
        ...
        this.alphaInactive =
            Color.alpha(this.colorInactive) / BottomNavigationItemViewAbstract.ALPHA_MAX
        this.alphaDisabled =
            Color.alpha(this.colorDisabled) / BottomNavigationItemViewAbstract.ALPHA_MAX
        this.alphaActive = Math.max(
            Color.alpha(colorActive).toFloat() / BottomNavigationItemViewAbstract.ALPHA_MAX,
            alphaInactive
        )
    ...
    override fun onStatusChanged(expanded: Boolean, size: Int, animate: Boolean) {
        ...
        val animator2 = ObjectAnimator.ofInt(
            this, "centerY", if (expanded) paddingBottomInactive else paddingTopItem,
    ...
    private fun updateLayoutOnAnimation(size: Int, fraction: Float, expanded: Boolean) {
        ...
        if (expanded) {
            ...
            icon!!.alpha =
                ((srcAlpha + fraction * (dstAlpha - srcAlpha)) * BottomNavigationItemViewAbstract.ALPHA_MAX).toInt()
            textPaint.alpha =
                (fraction * dstAlpha * BottomNavigationItemViewAbstract.ALPHA_MAX).toInt()
        } else {
            color = evaluator.evaluate(fraction, srcColor, dstColor) as Int
            val alpha = 1.0f - fraction
            icon!!.alpha =
                ((srcAlpha + alpha * (dstAlpha - srcAlpha)) * BottomNavigationItemViewAbstract.ALPHA_MAX).toInt()
            textPaint.alpha =
                (alpha * dstAlpha * BottomNavigationItemViewAbstract.ALPHA_MAX).toInt()
        }
