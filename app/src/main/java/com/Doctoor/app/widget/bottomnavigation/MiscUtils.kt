...
object MiscUtils {
    ...
    fun switchColor(
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            val currentAnimator =
                backgroundOverlay.getTag(R.id.bbn_backgroundOverlay_animator) as Animator?
    ...
    fun animate(
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            val startRadius = 10f
            val finalRadius =
                (if (centerX > navigation.width / 2) centerX else navigation.width - centerX).toFloat()
        ...
        if (ViewPropertyAnimatorCompat::class.java.isInstance(animator)) {
            (animator as ViewPropertyAnimatorCompat).setListener(object :
