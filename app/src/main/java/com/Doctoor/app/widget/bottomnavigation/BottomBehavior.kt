...
import com.Doctoor.app.widget.bottomnavigation.BottomNavigation.Companion.PENDING_ACTION_ANIMATE_ENABLED
import com.Doctoor.app.widget.bottomnavigation.BottomNavigation.Companion.PENDING_ACTION_NONE
...
open class BottomBehavior constructor(context: Context, attrs: AttributeSet) :
    ...
    init {
        ...
        this.animationDuration =
            array.getInt(
                ...
                context.resources.getInteger(R.integer.bbn_hide_animation_duration)
            )
        ...
        if (isSnackbar(dependency)) {
            if (null == snackbarDependentView) {
                snackbarDependentView =
                    SnackBarDependentView(dependency as SnackbarLayout, height, bottomInset)
            }
    ...
    override fun onStartNestedScroll(
        ...
        nestedScrollAxes: Int, @ViewCompat.NestedScrollType type: Int
    ): Boolean {
        ...
        if (nestedScrollAxes and ViewCompat.SCROLL_AXIS_VERTICAL != 0) {
            if (target.isScrollContainer && !target.canScrollVertically(-1) && !target.canScrollVertically(
                    1
                )
            ) {
                return false
    ...
    override fun onDirectionNestedPreScroll(
        ...
        if (offset > scaledTouchSlop) {
            handleDirection(
                coordinatorLayout,
                child,
                VerticalScrollingBehavior.ScrollDirection.SCROLL_DIRECTION_UP
            )
            offset = 0
            ...
            handleDirection(
                coordinatorLayout,
                child,
                VerticalScrollingBehavior.ScrollDirection.SCROLL_DIRECTION_DOWN
            )
            offset = 0
        }
    }
    ...
    private fun ensureOrCancelAnimator(
        coordinatorLayout: CoordinatorLayout,
        child: BottomNavigation
    ) {
        if (animator == null) {
    ...
    abstract class DependentView<V : View> internal constructor(
        protected val child: V, protected var height: Int,
    ...
    class SnackBarDependentView internal constructor(
        child: SnackbarLayout, height: Int,
        bottomInset: Int
