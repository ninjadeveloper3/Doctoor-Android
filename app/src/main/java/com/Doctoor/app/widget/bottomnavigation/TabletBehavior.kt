...
class TabletBehavior(context: Context, attrs: AttributeSet) :
    ...
    override fun onDependentViewChanged(
        ...
        val top =
            if (Build.VERSION.SDK_INT > 19) topInset else if (translucentStatus) topInset else 0
        params.topMargin =
            Math.max(dependency.top + dependency.height - top, if (translucentStatus) 0 else -top)
